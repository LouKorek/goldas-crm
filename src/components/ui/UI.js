  ...
  const dialog = state ? (
    <div className="modal-overlay" style={{zIndex:300}}>
      <div className="modal-box" style={{maxWidth:360}}>
        <p style={{color:'var(--text-1)',marginBottom:24,fontSize:14,lineHeight:1.6}}>{state.msg}</p>
...
// ── Date input — display as dd/mm/yyyy ────────────────────────────
...
export function EUBadge({ is }) {
  return (
    <span className="badge" style={{
      background: is?'rgba(96,165,250,0.15)':'var(--surface-3)',
      color: is?'var(--blue)':'var(--text-3)',
    }}>{is?'🇪🇺 EU':'Non-EU'}</span>
  );
}
...
// ── File upload with proper feedback ─────────────────────────────
export function FileUpload({ label, onUpload, history=[], accept='.pdf,.doc,.docx,.jpg,.png' }) {
  ...
  const doUpload = async (m) => {
    ...
    setUploading(true);
    try {
      await onUpload(file, name.trim(), m || mode || 'keep');
      setFile(null); setName(''); setMode(null);
    } catch(e) {
      console.error(e);
    }
    setUploading(false);
