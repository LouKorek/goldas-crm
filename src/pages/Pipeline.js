            ...
            {form.leagueMode==='select' ? (
              <div className="form-grid-2">
                <select value={f('leagueCountry')} onChange={e=>s('leagueCountry')(e.target.value)}>
                  ...
                  {['Israel','England','Spain','Germany','Italy','France','Portugal','Netherlands','Belgium','Turkey','Greece','Switzerland','Scotland','Russia','Ukraine','Serbia','Croatia','Czech Republic','Poland','Romania','Bulgaria','Argentina','Brazil'].map(c=><option key={c}>{c}</option>)}
                </select>
